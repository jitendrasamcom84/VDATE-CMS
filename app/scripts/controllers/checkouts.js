...
angular.module('vdateApp')
    ...
    .controller('ManageCheckOutsCtrl', function (Upload, $scope, ApiService, ApiEndpoint, $filter, $sce, $resource, _, alertService, $routeParams, $rootScope, AddressService, $location, api, DTOptionsBuilder, DTColumnDefBuilder) {
        $scope.checkoutsinfo = [];
        $scope.is_edit = false;
        ...
        var loadCheckoutsInfo = function () {
            ...
            ApiService.getModelViaPost(ApiEndpoint.URLS.CONDITIONS + ApiEndpoint.Models.CHECKOUTS, json).then(function (response) {
                if (response.data.is_error) {
                    alertService.error(response.data.message);
                } else {
                    $scope.checkoutsinfo = response.data.data;
                    for (var i = 0; i < $scope.checkoutsinfo.length; i++) {
                        if ($scope.checkoutsinfo[i].CardDueDate) {
                            $scope.checkoutsinfo[i].CardDueDate = $scope.checkoutsinfo[i].CardDueDate.slice(0, 2) + '/' + $scope.checkoutsinfo[i].CardDueDate.slice(2)
                        }
                    }
